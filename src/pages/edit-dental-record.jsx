...
export default function EditDentalRecord() {
  ...
  const [appointmentInterval, setAppointmentInterval] = useState(''); // NEW: State for the selected appointment interval
  
  // NEW: State for manual text inputs
  ...
  // NEW: Options for the appointment recall period
  ...
  const [formData, setFormData] = useState({
    ...
    treatmentDone: '',       // Add treatmentDone field
    calculus: '',
  ...
  const treatmentPlanOptions = [
    ...
    "Scaling and Polishing with Gross Stain",
    ...
    "Temporary Dressing",
    "Amalgam Filling",
    "Fuji 9 (Posterior GIC (per Filling)",
    "Tooth Whitening (3 Sessions)",
    "Curretage/Subgingival (per tooth)",
    "Composite Buildup",
    "Removable Denture (Additional Tooth)",
    "PFM Crown",
    ...
    "Incision & Drainage/Suturing with Debridement",
    "Fissure Sealant",
    ...
    "Essix Retainer",
    "Crown Cementation",
    "Esthetic Tooth Filling",
    "Zirconium Crown",
    "Gold Crown",
    "Flexible Denture (per tooth)",
    "Flexible Denture (2nd tooth)",
    "Metallic Crown",
    "Dental Implant – One Tooth",
    "Dental Implant – Two Teeth",
      ...
      try {
        ...
        if (recordResponse.ok) {
          ...
          setFormData({
            ...
            medicationHIV: recordData.medicationHIV || false,
            medicationHepatitis: recordData.medicationHepatitis || false,
            ...
            treatmentDone: recordData.treatmentDone || '',
            calculus: recordData.calculus || '',
            recordDate: recordData.createdAt ? new Date(recordData.createdAt).toISOString().split('T')[0] : new Date().toISOString().split('T')[0],
          });
  ...
  const handleAddProvisionalDiagnosis = () => {
    ...
    // NEW: Handler for manual diagnosis input
  const handleAddManualProvisionalDiagnosis = () => {
    ...
    if (trimmedDiagnosis && !formData.provisionalDiagnosis.includes(trimmedDiagnosis)) {
        setFormData(prevData => ({
            ...prevData,
            provisionalDiagnosis: [...prevData.provisionalDiagnosis, trimmedDiagnosis]
        }));
        setManualProvisionalDiagnosis(''); // Clear input after adding
    }
  };
  ...
  // NEW: Handler for manual treatment plan input
  const handleAddManualTreatmentPlan = () => {
    ...
    if (trimmedPlan && !formData.treatmentPlan.includes(trimmedPlan)) {
        setFormData(prevData => ({
            ...prevData,
            treatmentPlan: [...prevData.treatmentPlan, trimmedPlan]
        }));
        setManualTreatmentPlan(''); // Clear input after adding
    }
  };
...
const handleSubmit = async (e) => {
    ...
    if (!formData.complaint || formData.provisionalDiagnosis.length === 0 || formData.treatmentPlan.length === 0 || !formData.treatmentDone) {
      setMessage('Please fill in Chief Complaint, Provisional Diagnosis, Treatment Plan, and Treatment Done.');
      setIsError(true);
      setSubmitting(false);
      return;
    }
    ...
    if (!appointmentInterval) {
        setMessage('Please set the next appointment recall period before submitting.');
        setIsError(true);
        setSubmitting(false);
        return;
    }
    ...
    try {
      // Step 1: Update the dental record
      const payload = {
        ...formData,
        updatedAt: new Date(),
      };
      ...
      if (!response.ok) {
        const errorData = await response.json().catch(() => ({ error: 'Failed to update dental record. Server returned a non-JSON response.' }));
        throw new Error(errorData.error);
      }
      
      // Step 2: Schedule the next appointment
      ...
      if (!appointmentResponse.ok) {
          setMessage(`Record updated, but failed to set appointment: ${appointmentData.error}. Please set it manually.`);
          setIsError(true);
      } else {
          setMessage('Dental record updated and next appointment scheduled successfully!');
          setIsError(false);
          setTimeout(() => navigate(`/patients/${patientId}`), 2000); // Redirect to patient details page
      }
      ...
      <form onSubmit={handleSubmit}>
        ...
        <section className="form-section">
          ...
          <div className="checkbox-group">
            ...
            <div className="checkbox-item">
              <input type="checkbox" id="medicationHIV" name="medicationHIV" checked={formData.medicationHIV} onChange={handleChange} />
              <label htmlFor="medicationHIV">HIV</label>
            </div>
            <div className="checkbox-item">
              <input type="checkbox" id="medicationHepatitis" name="medicationHepatitis" checked={formData.medicationHepatitis} onChange={handleChange} />
              <label htmlFor="medicationHepatitis">Hepatitis</label>
            </div>
          </div>
        </section>
          ...
          <div className="form-grid">
            ...
            <div className="form-group">
              ...
              <div className="quadrant-input-group">
                <div className="quadrant-input-item"><span className="quadrant-label">Q1</span><input type="text" name="teethPresent.q1" value={formData.teethPresent.q1} onChange={handleChange} /></div>
                <div className="quadrant-input-item"><span className="quadrant-label">Q2</span><input type="text" name="teethPresent.q2" value={formData.teethPresent.q2} onChange={handleChange} /></div>
                <div className="quadrant-input-item"><span className="quadrant-label">Q4</span><input type="text" name="teethPresent.q4" value={formData.teethPresent.q4} onChange={handleChange} /></div>
                <div className="quadrant-input-item"><span className="quadrant-label">Q3</span><input type="text" name="teethPresent.q3" value={formData.teethPresent.q3} onChange={handleChange} /></div>
              </div>
            </div>
            ...
            <div className="form-group">
              ...
              <div className="quadrant-input-group">
                <div className="quadrant-input-item"><span className="quadrant-label">Q1</span><input type="text" name="cariousCavity.q1" value={formData.cariousCavity.q1} onChange={handleChange} /></div>
                <div className="quadrant-input-item"><span className="quadrant-label">Q2</span><input type="text" name="cariousCavity.q2" value={formData.cariousCavity.q2} onChange={handleChange} /></div>
                <div className="quadrant-input-item"><span className="quadrant-label">Q4</span><input type="text" name="cariousCavity.q4" value={formData.cariousCavity.q4} onChange={handleChange} /></div>
                <div className="quadrant-input-item"><span className="quadrant-label">Q3</span><input type="text" name="cariousCavity.q3" value={formData.cariousCavity.q3} onChange={handleChange} /></div>
              </div>
            </div>
            ...
            <div className="form-group">
              ...
              <div className="quadrant-input-group">
                <div className="quadrant-input-item"><span className="quadrant-label">Q1</span><input type="text" name="filledTeeth.q1" value={formData.filledTeeth.q1} onChange={handleChange} /></div>
                <div className="quadrant-input-item"><span className="quadrant-label">Q2</span><input type="text" name="filledTeeth.q2" value={formData.filledTeeth.q2} onChange={handleChange} /></div>
                <div className="quadrant-input-item"><span className="quadrant-label">Q4</span><input type="text" name="filledTeeth.q4" value={formData.filledTeeth.q4} onChange={handleChange} /></div>
                <div className="quadrant-input-item"><span className="quadrant-label">Q3</span><input type="text" name="filledTeeth.q3" value={formData.filledTeeth.q3} onChange={handleChange} /></div>
              </div>
            </div>
            ...
            <div className="form-group">
              ...
              <div className="quadrant-input-group">
                <div className="quadrant-input-item"><span className="quadrant-label">Q1</span><input type="text" name="missingTeeth.q1" value={formData.missingTeeth.q1} onChange={handleChange} /></div>
                <div className="quadrant-input-item"><span className="quadrant-label">Q2</span><input type="text" name="missingTeeth.q2" value={formData.missingTeeth.q2} onChange={handleChange} /></div>
                <div className="quadrant-input-item"><span className="quadrant-label">Q4</span><input type="text" name="missingTeeth.q4" value={formData.missingTeeth.q4} onChange={handleChange} /></div>
                <div className="quadrant-input-item"><span className="quadrant-label">Q3</span><input type="text" name="missingTeeth.q3" value={formData.missingTeeth.q3} onChange={handleChange} /></div>
              </div>
            </div>
            ...
            <div className="form-group">
              ...
              <div className="quadrant-input-group">
                <div className="quadrant-input-item"><span className="quadrant-label">Q1</span><input type="text" name="fracturedTeeth.q1" value={formData.fracturedTeeth.q1} onChange={handleChange} /></div>
                <div className="quadrant-input-item"><span className="quadrant-label">Q2</span><input type="text" name="fracturedTeeth.q2" value={formData.fracturedTeeth.q2} onChange={handleChange} /></div>
                <div className="quadrant-input-item"><span className="quadrant-label">Q4</span><input type="text" name="fracturedTeeth.q4" value={formData.fracturedTeeth.q4} onChange={handleChange} /></div>
                <div className="quadrant-input-item"><span className="quadrant-label">Q3</span><input type="text" name="fracturedTeeth.q3" value={formData.fracturedTeeth.q3} onChange={handleChange} /></div>
              </div>
            </div>

            <div className="form-group">
              <label htmlFor="periodontalCondition">Periodontal Condition</label>
              <select id="periodontalCondition" name="periodontalCondition" value={formData.periodontalCondition} onChange={handleChange}>
            ...
            <div className="form-group">
              ...
              <select id="oralHygiene" name="oralHygiene" value={formData.oralHygiene} onChange={handleChange}>
          ...
          <div className="form-grid">
            ...
            <div className="form-group full-width-grid-item">
              ...
              <div className="add-remove-control">
                <select id="provisionalDiagnosisSelect" value={selectedProvisionalDiagnosisOption} onChange={(e) => setSelectedProvisionalDiagnosisOption(e.target.value)} className="form-select">
                  <option value="">Select a diagnosis from the list</option>
                  {provisionalDiagnosisOptions.map((option) => (<option key={option} value={option}>{option}</option>))}
                </select>
                <button type="button" onClick={handleAddProvisionalDiagnosis} className="add-button">Add Selected</button>
              </div>
              <div className="add-remove-control" style={{ marginTop: '10px' }}>
                <input
                  type="text"
                  placeholder="Or type a new diagnosis manually"
                  value={manualProvisionalDiagnosis}
                  onChange={(e) => setManualProvisionalDiagnosis(e.target.value)}
                />
                <button type="button" onClick={handleAddManualProvisionalDiagnosis} className="add-button">
                  Add Manual
                </button>
              </div>
              {formData.provisionalDiagnosis.length > 0 && (
                <ul className="added-items-list">
                  {formData.provisionalDiagnosis.map((diagnosis, index) => (
                    <li key={index}>
                      {diagnosis}
                      <button type="button" onClick={() => handleRemoveProvisionalDiagnosis(diagnosis)} className="remove-item-button"><i className="fas fa-trash"></i></button>
                    </li>
                  ))}
                </ul>
              )}
            ...
            <div className="form-group full-width-grid-item">
              ...
              <div className="add-remove-control">
                <select id="treatmentPlanSelect" value={selectedTreatmentPlanOption} onChange={(e) => setSelectedTreatmentPlanOption(e.target.value)} className="form-select">
                  <option value="">Select a treatment plan from the list</option>
                  {treatmentPlanOptions.map((option) => (<option key={option} value={option}>{option}</option>))}
                </select>
                <button type="button" onClick={handleAddTreatmentPlan} className="add-button">Add Selected</button>
              </div>
              <div className="add-remove-control" style={{ marginTop: '10px' }}>
                <input
                  type="text"
                  placeholder="Or type a new treatment plan manually"
                  value={manualTreatmentPlan}
                  onChange={(e) => setManualTreatmentPlan(e.target.value)}
                />
                <button type="button" onClick={handleAddManualTreatmentPlan} className="add-button">
                  Add Manual
                </button>
              </div>
              {formData.treatmentPlan.length > 0 && (
                <ul className="added-items-list">
                  {formData.treatmentPlan.map((plan, index) => (
                    <li key={index}>
                      {plan}
                      <button type="button" onClick={() => handleRemoveTreatmentPlan(plan)} className="remove-item-button"><i className="fas fa-trash"></i></button>
                    </li>
                  ))}
                </ul>
              )}
